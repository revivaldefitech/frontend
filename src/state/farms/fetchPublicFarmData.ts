...
type PublicFarmData = {
  ...
  multiplier: string
  depositfee: string
  vivalperblock: SerializedBigNumber
  canharvest: boolean
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, vivalPerBlock] =
    pid || pid === 0
  ...
  const VivalPerBlock = vivalPerBlock ? new BigNumber(vivalPerBlock).div(BIG_TEN.pow(18)) : BIG_ZERO
  ...
  return {
    ...
    vivalperblock: VivalPerBlock.toJSON(),
    canharvest
  }
}
